...
local function _Includes()
    ...
    require("Envir/QuestDiary/CCLua/GameModule/EquipCompose")
...
function _TempTestCode()
    release_print('_TempTestCode 运行中，正式关闭----------------------------')

--[[
    local infoStr = ''
    for i = 10, 1, -1 do
        release_print('xxxxxx i is '..i)

        local infoTab = {}
        if infoStr ~= '' then
            infoTab = json2tbl(infoStr)
        end
        if infoTab[i] == nil then    
            infoTab[i] = i
            infoStr = tbl2json(infoTab)   

            release_print('init infoTab[i]: i is '..i)
            release_print('init infoStr is '..infoStr)
        end
        infoTab[i] = i * 10
        infoStr = tbl2json(infoTab)
        release_print('multi 10 infoStr is '..infoStr)
    end    
]]--    

--[[
    local infoStr = '' -- 假设空的 JSON 对象是一个空的大括号对
    local infoTab = {} -- 将空的 JSON 对象转换为表
    ...
    for i = 10, 1, -1 do
        ...
        if infoTab[sid] == nil then
            infoTab[sid] = i * 10 -- 直接设置为 i * 10
            release_print('init infoTab[i]: i is '..sid)        
        end

        infoStr = tbl2json(infoTab) -- 最后将整个表转换为 JSON 字符串
        release_print('xxxxxx infoStr is '..infoStr)
    end
]]--
